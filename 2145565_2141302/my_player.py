...
class MyPlayer(PlayerDivercite):
    ...
    def compute_action(self, current_state: GameState, remaining_time: int = 1e9, **kwargs) -> Action:
        ...
        action = self.minimaxSearch(current_state)
        return action

################ Minmax part ################

    def minimaxSearch(self, state: GameState):
        v, m = self.maxValue(state, 0)
        return m


    def maxValue(self, state: GameState, counter:int, alpha = -1000000, beta = 1000000):
        ...
        for action in self.getPossibleActions(state):
            ...
            counter += 1
            v, _ = self.minValue(temporary_state, counter, alpha, beta)
            if  v > v_star:
                v_star = v
                m_star = action
    ...
    def minValue(self, state: GameState, counter: int, alpha = -1000000, beta = 1000000):
        # We use transposition table to avoid redundant calculations here
        state_hash = None
        if self.use_transposition_table:   
            state_hash = self.hash_state(state)
    ...
    def transition(self, action: Action, state: GameState):
        """
        Apply a given action to the current game state and return the resulting state.
        ...
        Returns:
            GameState: The next game state after applying the action.
        """
    ...
    def get_opponent_id(self, current_state:GameState):
        """
        Retrieve the ID of the opponent player.
...
################ Heuristic part ################
    ...
    def evaluation(self, state:GameStateDivercite, score:int) -> float:
        """
        Evaluate the current game state based on various factors, such as resources, divercities, and blocking potential.
        ...
        Returns:
            float: The evaluation score of the game state, considering multiple weighted factors.
        """
    ...
    def is_city_placement(self, action: LightAction):
        """
        Check if the given action corresponds to placing a city.

        Args:
            action (LightAction): The action to evaluate.

        Returns:
            int: Returns 1 if the action places a city, otherwise 0.
        """
    ...
    def max_available_resource(self, state:GameStateDivercite, action: LightAction):
        """
        Determine the maximum availability of a resource related to a given action.

        Args:
            state (GameStateDivercite): The current state of the game.
            action (LightAction): The action being evaluated.

        Returns:
            int: The number of remaining pieces for the resource if the action places a resource piece; otherwise, 0.
        """
    ...
    def is_divercity(self, all_actions_divercity: List[LightAction], action: LightAction):
        """
        Check if a given action is a divercity action.

        Args:
            all_actions_divercity (List[LightAction]): A list of all possible divercity actions.
            action (LightAction): The action to evaluate.

        Returns:
            int: Returns 1 if the action is a divercity action, otherwise 0.
        """
    ...
    def get_minimum_unique_resource(self, current_state: GameStateDivercite, player_id: int):
        # Counting the number of unique resources left for the player
        my_remaining_pieces =  current_state.players_pieces_left.get(player_id, {})
    ...
    def get_amount_divercity(self, state: GameStateDivercite, player_id: int):
        """
        Count the number of divercities left for a given player.
        ...
        Returns:
            int: The number of divercities left for the player.
        """
